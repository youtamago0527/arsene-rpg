# 或世盗 -ARSÈNE- RE:MIX — Ver.1.0

> 奪え、編み直せ、物語を。 ―― STEAL / REWRITE / REMIX

JOBを育て、武器学を磨き、盗んだ力で異世界へ潜るブラウザ怪盗RPGです。インストール不要でPC・スマートフォンの両方に対応し、ダンジョン3種・ボス戦・装備の製作と強化・JOBチェンジ・異世界（ファントムシーフ）まで遊べます。
...
Node.js があれば `pnpm play`（または `node scripts/play.mjs`）でローカルサーバーの起動とブラウザの自動オープンをまとめて行えます。ポートを変える場合は `pnpm play -- --port=3000`、ブラウザを自動で開かない場合は `pnpm play -- --no-open` を指定してください。

...
(function(){
  ...
  const api = {
    ...
    show(type='auto2', extra={}){
      ...
      const modal=document.createElement('div'); modal.className='q-offer-modal'; modal.innerHTML=type==='revive'?reviveCard:normalCard;
      if(type==='revive') modal.classList.add('q-offer-defeat');
      document.body.appendChild(modal); let n=3; const count=modal.querySelector('[data-q-countdown]'); const timer=setInterval(()=>{n--; if(count)count.textContent=n; if(n<=0){clearInterval(timer); modal.querySelector('[data-q-watch]').disabled=false;}},1000); modal.querySelector('[data-q-watch]').disabled=true;
      modal.addEventListener('click',e=>{if(e.target.closest('[data-q-close]')){clearInterval(timer);modal.remove();if(typeof d.onClose==='function')d.onClose();return;} if(e.target.closest('[data-q-watch]')&&!e.target.closest('[data-q-watch]').disabled){clearInterval(timer);const granted=api.grant(type);modal.remove();if(granted&&typeof d.onGrant==='function')d.onGrant();}});
      return true;
    },

  ...
  class StartFlow {
    ...
    async init() {
      try {
        [this.prologue, this.characters] = await Promise.all([
          ...
          fetch('data/characters.json?v=0.4.13').then(response => response.json())
        ]);
      } catch (error) {
    ...
    render() {
      this.root.innerHTML = `
        ...
        <section class="flow-screen title-screen" data-flow-screen="title">
          <div class="flow-frame"><div class="title-content"><img class="title-logo" src="assets/ui/brand/logo-800.webp" srcset="assets/ui/brand/logo-800.webp 800w, assets/ui/brand/logo.webp 1200w" sizes="(max-width:760px) 88vw, 620px" width="800" height="450" alt="或世盗 -ARSÈNE- RE:MIX" fetchpriority="high"><p>PHANTOM THIEF RPG</p><nav class="title-menu"><button data-title-new>NEW GAME<span>新たな物語を始める</span></button><button data-title-continue>CONTINUE<span>セーブデータから再開</span></button><button data-title-settings>SETTINGS<span>音量・データ設定</span></button></nav><small class="early-access-note">開発中のEarly Access版です。更新によりバランス・機能・セーブ仕様が変更される場合があります。</small></div><div class="flow-version">EARLY ACCESS Ver.0.1<br>DUNGEON 1–5 AVAILABLE</div></div>
        </section>
    ...
    bind() {
      this.root.addEventListener('click', async event => {
        ...
        if (event.target.closest('[data-ad-privacy-options]')) { const shown = await window.arseneAdMob?.showPrivacyOptions?.(); this.toast(shown ? 'プライバシー設定を開きました' : '現在はプライバシー設定を開けません'); return; }

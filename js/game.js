  ...
  class BattleGame {
    ...
    */
    ...
    async defeat() {
      ...
      this.flashTitle('GAME OVER', 'MISSION FAILED'); this.showResult('GAME OVER', '戦闘不能になった。カズに救助され、HP1で拠点へ運び込まれた。', 'RETURN TO HIDEOUT', this.battleSummaryHTML() || ''); this.showReviveOfferIfAvailable();
    }
    showDefeatWithRevive(copy, kicker = 'CHALLENGE FAILED') { this.showResult('GAME OVER', copy, kicker, this.battleSummaryHTML() || '<div class="boss-result-note">報酬・ドロップなし</div>'); this.showReviveOfferIfAvailable(); }
    showReviveOfferIfAvailable() { const offer = window.arseneQOffer; if (!offer?.canUse?.('revive')) return false; return offer.show('revive', { onGrant: () => this.reviveAfterDefeat() }); }

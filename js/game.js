(() => {
  ...
  class BattleGame {
    constructor() {
      this.profile = this.loadProfile(); this.syncSkillUnlocks(); this.player = null; this.enemies = []; this.turn = 1; this.locked = false; this.finished = false; this.selectedEquipmentId = null; this.battleMode = 'slime'; this.workshopTab = 'weapon'; this.craftDungeonFilter = 'all'; this.craftArmorFilter = 'leftHand';
    ...
    freshProfile() {
      const p = D.player; return { version: 6, selectedCharacter: null, playerCharacter: null, prologueCompleted: false, openingWatched: false, level: p.level, exp: p.exp, gold: p.gold, baseStats: clone(p.baseStats), currentVitals: { hp: p.baseStats.maxHp, mp: p.baseStats.maxMp }, equipment: clone(p.equipment), inventory: clone(p.inventory), musicScores: {}, bossDefeated: { zenacad: false }, currentJob: 'mage', jobs: { warrior: { level: 1, exp: 0 }, mage: { level: 1, exp: 0 }, martialArtist: { level: 1, exp: 0 }, priest: { level: 1, exp: 0 } }, learnedJobSkills: [], learnedCharacterSkills: ['blueNote'], activeSkills: ['blueNote', 'quickSlash'], flags: { noelFirstEncounterCleared: false, preNoelBattleWins: 0, postNoelBattleWins: 0, zenakadoDefeated: false, zenakadoScoreClaimed: false, ramenBuffActive: false, normalBattleWins: 0, temporaryBossCompleted: false, openingWatched: false, prologueCompleted: false, dungeon2BattleWins: 0, dungeon2NewSeen: false }, discoveredMaterials: [], unlockedRecipes: [], newlyUnlockedRecipes: [] };
    }
    loadProfile() {
      try {
    ...
    syncSkillUnlocks() { const learnedCharacter = new Set(this.profile.learnedCharacterSkills || []), learnedJob = new Set(this.profile.learnedJobSkills || []); (D.characterSkillProgression || []).forEach(entry => { if (this.profile.level >= entry.level) learnedCharacter.add(entry.skillId); }); Object.entries(this.profile.jobs || {}).forEach(([jobId, progress]) => { const job = D.jobs[jobId]; Object.entries(job?.skillUnlocks || {}).forEach(([level, skillId]) => { if (progress.level >= Number(level)) learnedJob.add(skillId); }); }); this.profile.learnedCharacterSkills = [...learnedCharacter]; this.profile.learnedJobSkills = [...learnedJob]; const allowed = new Set(['quickSlash', ...learnedCharacter, ...learnedJob]); this.profile.activeSkills = (this.profile.activeSkills || []).filter(id => allowed.has(id) && D.skills[id]?.type !== 'PASSIVE').slice(0, 4); for (const id of learnedCharacter) if (D.skills[id]?.type === 'ACTIVE' && this.profile.activeSkills.length < 4 && !this.profile.activeSkills.includes(id)) this.profile.activeSkills.push(id); }
    ...
    equipmentBonuses(equipment = this.profile.equipment) {
      const result = {}; const add = source => Object.entries(source?.bonuses || {}).forEach(([k, v]) => result[k] = (result[k] || 0) + v);
      Object.values(equipment).forEach(id => add(this.equipmentDefinition(id))); return result;
    }
    ...
    totalStats(equipment = this.profile.equipment) {
      const total = clone(this.profile.baseStats), bonuses = this.equipmentBonuses(equipment), jobBonuses = this.activeJobBonuses(); Object.entries(bonuses).forEach(([k, v]) => total[k] = (total[k] || 0) + v); Object.entries(jobBonuses).forEach(([k, v]) => total[k] = (total[k] || 0) + v); const setEffects = this.activeSetEffects(equipment); if (setEffects.magPercent) total.mag = Math.max(total.mag + 1, Math.floor(total.mag * (1 + setEffects.magPercent / 100))); if (this.profile.flags.ramenBuffActive) total.maxHp = Math.ceil(total.maxHp * 1.03); total.critBonus ||= 0; total.def = total.vit; return total;
    }
    getDungeon(id = this.currentDungeonId) { return (D.dungeons || []).find(d => d.id === id) || (D.dungeons || [])[0]; }
    isDungeonUnlocked(id) { const d = this.getDungeon(id); if (!d) return false; if (!d.unlockCondition) return true; if (d.unlockCondition === 'dungeon1Clear') return this.isBossDefeated('zenacad'); return false; }
    ...
    startBattle() {
      ...
      const dungeon = this.getDungeon(), dungeon2 = this.currentDungeonId !== 'dungeon1';
      let wins, lineup;
      if (dungeon2) {
        wins = this.profile.flags.dungeon2BattleWins || 0;
    ...
    updateHUD() {
      const p = this.player; $('#player-hp').textContent = `${p.hp} / ${p.stats.maxHp}`; $('#player-mp').textContent = `${p.mp} / ${p.stats.maxMp}`; $('#player-hp-bar').style.width = `${100 * p.hp / p.stats.maxHp}%`; $('#player-mp-bar').style.width = `${100 * p.mp / p.stats.maxMp}%`; $('#level').textContent = this.profile.level; $('#turn-label').textContent = `TURN ${String(this.turn).padStart(2, '0')}`;
    ...
    showMainCommands() { $('#phase-label').textContent = 'COMMAND'; const itemCount = (this.profile.inventory.potion || 0) + (this.profile.inventory.manaPotion || 0); this.panel(this.button('たたかう', 'ATTACK', 'attack') + this.button('スキル', 'SKILL', 'skill') + this.button('アイテム', `ITEM ×${itemCount}`, 'item') + this.button('にげる', 'ESCAPE', 'escape')); this.bindActions({ attack: () => this.chooseTarget('attack'), skill: () => this.showSkills(), item: () => this.showBattleItems(), escape: () => this.tryEscape() }); }
    ...
    availableSkills() { const ids = new Set(this.profile.activeSkills || []); const grant = this.equippedWeapon()?.grantsSkillId; if (grant) ids.add(grant); return [...ids].map(id => D.skills[id]).filter(Boolean); }
    cooldownRemaining(skill) { return Math.max(0, (this.player.cooldowns?.[skill.id] || 0) - this.turn); }
    showSkills() { const skills = this.availableSkills(); this.panel(skills.map(s => { const cooldown = this.cooldownRemaining(s); return this.button(s.name, cooldown ? `CT ${cooldown}` : `MP ${s.mp}`, s.id, this.player.mp < s.mp || cooldown > 0); }).join('') + this.button('もどる', 'BACK', 'back')); const actions = { back: () => this.showMainCommands() }; skills.forEach(s => { actions[s.id] = () => this.chooseTarget(s.id); }); this.bindActions(actions); }
    ...
    async executeRound(skillId, targetIndex) {
      ...
      for (const actor of actors) { if (this.finished || this.player.hp <= 0) break; if (actor.type === 'enemy' && !actor.enemy.alive) continue; await actor.act(); await sleep(300); if (!this.enemies.some(e => e.alive)) { await this.victory(); return; } }
    ...
    async beginPlayerTurn() { if (this.characterHasSkill('blueEcho') && Math.random() < .20) { this.player.buffs.blueEcho = 2; this.flashTitle('BLUE ECHO', 'MAG +10% // 2 TURNS'); this.setLog('蒼の残響が魔力を高める！'); await sleep(260); } if ((this.player.buffs.regenerate || 0) > 0) { const heal = Math.max(1, Math.ceil(this.player.stats.maxHp * .08)), gained = Math.min(heal, this.player.stats.maxHp - this.player.hp); this.player.hp += gained; if (gained) { this.audio.sfx('heal'); this.floating($('#ren'), `+${gained}`, 'heal'); this.setLog(`リジェネレートでHPが${gained}回復！`); this.updateHUD(); await sleep(220); } } }
    endPlayerTurn() { if ((this.player.buffs.blueEcho || 0) > 0) this.player.buffs.blueEcho--; if ((this.player.buffs.regenerate || 0) > 0) this.player.buffs.regenerate--; }
    damageFor(skill, enemy) {
      const s = this.player.stats, w = this.equippedWeapon(), balance = D.combatBalance, weaponAttack = skill.id === 'attack', statKey = weaponAttack ? w.damageStat : (skill.kind === 'magical' ? 'mag' : 'str'), stat = this.effectivePlayerStat(statKey), power = weaponAttack ? w.power : skill.power, defDown = this.turn <= (enemy.defDownUntil || 0) ? .15 : 0, effectiveDef = enemy.stats.def * (1 - defDown) * (1 - (skill.ignoreDef || 0));
    ...
    async playerAction(skill, targetIndex) { await this.playerAttack(skill, targetIndex); const repeatChance = this.activeSetEffects().magicRepeatChance || 0; if (skill.kind === 'magical' && this.enemies.some(e => e.alive) && Math.random() < repeatChance) { this.flashTitle('《独奏曲》', 'CADENZA // ENCORE'); this.setLog('ゼナカドの旋律が魔法を再演する！'); await sleep(360); await this.playerAttack(skill, targetIndex); } }
    async playerAttack(skill, targetIndex) {
      ...
      const ren = $('#ren'), el = document.getElementById(target.uid), hits = skill.hits || 1; ren.classList.add(staffAttack || skill.kind === 'magical' || skill.kind === 'hybrid' ? 'casting' : 'attacking'); if (staffAttack || skill.kind === 'magical' || skill.kind === 'hybrid') { if (staffAttack) this.flashTitle('MAGIC SHOT', w.name); await sleep(220); await this.magicProjectile(el); } else await sleep(220); let total = 0, criticals = 0; for (let hit = 0; hit < hits && target.hp > 0; hit++) { el.classList.add('hit'); const d = this.damageFor(skill, target); total += d.value; if (d.critical) criticals++; target.hp = target.cannotDefeat ? Math.max(1, target.hp - d.value) : Math.max(0, target.hp - d.value); this.floating(el, d.value, d.critical ? 'critical' : 'damage'); this.audio.sfx(d.critical ? 'critical' : 'enemyHit'); this.updateHUD(); await sleep(hits > 1 ? 190 : 420); el.classList.remove('hit'); } this.setLog(`${criticals ? `CRITICAL ×${criticals}! ` : ''}${target.name}${target.label}に${total}ダメージ！${hits > 1 ? `（${hits}HIT）` : ''}`); ren.classList.remove('attacking', 'casting');
      ...
      if (target.hp <= 0) { target.alive = false; this.audio.sfx('defeat'); el.classList.add('defeated'); target.rolledDrops = this.rollDrops(target); target.rolledDrops.forEach(([id]) => { const item = D.items[id]; if (item) this.floating(el, item.name, 'heal'); }); const earned = this.grantEnemyReward(target); this.setLog(`${target.name}${target.label}を撃破！ EXP+${earned.exp} GOLD+${earned.gold}`); await sleep(600); }
    }
    async applySelfSkill(skill) { this.flashTitle(skill.name, skill.nameEn || 'SELF SKILL'); const effect = skill.effect || {}, ren = $('#ren'); ren.classList.add('casting'); await sleep(260); if (effect.type === 'mpRecover') { const amount = Math.max(1, Math.ceil(this.player.stats.maxMp * effect.maxMpRate)), gained = Math.min(amount, this.player.stats.maxMp - this.player.mp); this.player.mp += gained; this.audio.sfx('heal'); this.floating(ren, `MP +${gained}`, 'heal'); this.setLog(`精神集中でMPが${gained}回復！`); } if (effect.type === 'hpRecover') { const amount = Math.max(1, Math.round(this.player.stats.mnd * effect.mndScale + effect.base)), gained = Math.min(amount, this.player.stats.maxHp - this.player.hp); this.player.hp += gained; this.audio.sfx('heal'); this.floating(ren, `+${gained}`, 'heal'); this.setLog(`ヒールでHPが${gained}回復！`); } if (effect.type === 'regenerate') { this.player.buffs.regenerate = effect.turns + 1; this.audio.sfx('heal'); this.setLog('リジェネレート！ 3ターンの間、HPが回復する。'); } this.persistVitals(); this.updateHUD(); await sleep(350); ren.classList.remove('casting'); }
    async playerAttackAll(skill) {
      ...
      for (const target of targets) {
        ...
        await sleep(220); el.classList.remove('hit');
        if (target.hp <= 0) { target.alive = false; this.audio.sfx('defeat'); el.classList.add('defeated'); target.rolledDrops = this.rollDrops(target); target.rolledDrops.forEach(([id]) => { const item = D.items[id]; if (item) this.floating(el, item.name, 'heal'); }); this.grantEnemyReward(target); }
      }
      ren.classList.remove('casting'); const defeatedNames = targets.filter(t => !t.alive).map(t => `${t.name}${t.label}`);
      this.setLog(defeatedNames.length ? `${defeatedNames.join('、')}を撃破！` : `${skill.name}が敵全体を襲う！`); await sleep(400);
    }
    async magicProjectile(targetEl) { const field = $('#battlefield').getBoundingClientRect(), from = $('#weapon-layer').getBoundingClientRect(), to = targetEl.getBoundingClientRect(), orb = document.createElement('i'), sx = from.right - field.left, sy = from.top - field.top + from.height * .22, ex = to.left - field.left + to.width * .48, ey = to.top - field.top + to.height * .58; orb.className = 'magic-projectile'; orb.style.left = `${sx}px`; orb.style.top = `${sy}px`; orb.style.setProperty('--shot-x', `${ex - sx}px`); orb.style.setProperty('--shot-y', `${ey - sy}px`); $('#battlefield').appendChild(orb); await sleep(460); orb.remove(); }
    async enemyAttack(enemy) {
      ...
      this.setLog(`${enemy.name}${enemy.label}の${chosen.name}！`); if (isMagic) { this.flashTitle(chosen.name, 'SHADOW MAGIC'); this.audio.sfx('dark'); } const el = document.getElementById(enemy.uid), ren = $('#ren'); el.classList.add('enemy-attacking'); await sleep(300); ren.classList.add('hit');
      ...
      if (miss) { this.floating(ren, 'MISS', 'miss'); this.setLog('RENは攻撃をかわした！'); } else { this.audio.sfx('playerHit'); this.player.hp = Math.max(0, this.player.hp - damage); this.persistVitals(); this.floating(ren, damage, 'enemy-damage'); this.setLog(`RENは${damage}ダメージを受けた！`); } this.updateHUD(); await sleep(420); el.classList.remove('enemy-attacking'); ren.classList.remove('hit');
    }
    async bossAttack(enemy) {
      if (enemy.cannotDefeat) {
        const el = document.getElementById(enemy.uid), ren = $('#ren'); this.setLog(`${enemy.name}のエターナル・ジャッジメント！`); this.flashTitle('裁定の刻', 'ETERNAL JUDGEMENT'); this.audio.sfx('dark'); el.classList.add('enemy-attacking'); await sleep(520); ren.classList.add('hit'); const damage = Math.max(this.player.hp, Math.round(enemy.stats.mag * 1.5)); this.player.hp = 0; this.persistVitals(); this.audio.sfx('critical'); this.floating(ren, damage, 'enemy-damage'); this.setLog('圧倒的な裁定の前に、RENは膝をついた……'); this.updateHUD(); await sleep(650); el.classList.remove('enemy-attacking'); ren.classList.remove('hit'); return;
      }
      let r = Math.random(), chosen = enemy.ai[enemy.ai.length - 1], acc = 0;
      for (const s of enemy.ai) { acc += s.weight; if (r < acc) { chosen = s; break; } }
      ...
      el.classList.add('enemy-attacking'); await sleep(400); ren.classList.add('hit');
      const balance = D.combatBalance, formula = isMagic ? balance.enemyMagic : balance.enemyPhysical;
      const attackStat = isMagic ? enemy.stats.mag : enemy.stats.atk, defenseStat = isMagic ? this.player.stats.mnd : this.player.stats.def * (this.turn <= (this.player.defDownUntil || 0) ? .8 : 1);
      ...
      this.audio.sfx('playerHit'); this.player.hp = Math.max(0, this.player.hp - damage); this.persistVitals(); this.floating(ren, damage, 'enemy-damage'); this.setLog(`RENは${damage}ダメージを受けた！`); this.updateHUD(); await sleep(450); el.classList.remove('enemy-attacking'); ren.classList.remove('hit');
    }
    ...
    async useConsumable(id) { const item = D.items[id], amount = item?.effect?.hp || item?.effect?.mp || 0, key = item?.effect?.hp ? 'hp' : 'mp', maxKey = key === 'hp' ? 'maxHp' : 'maxMp'; if (!item || !(this.profile.inventory[id] > 0)) { this.setLog(`${item?.name || 'アイテム'}を持っていない。`); return; } if (this.player[key] >= this.player.stats[maxKey]) { this.setLog(`${key.toUpperCase()}は満タンだ。`); return; } this.locked = true; this.panel(''); await this.beginPlayerTurn(); const heal = Math.min(amount, this.player.stats[maxKey] - this.player[key]); this.profile.inventory[id]--; this.player[key] += heal; this.persistVitals(); this.audio.sfx('heal'); this.setLog(`${item.name}を使った。${key.toUpperCase()}が${heal}回復！`); this.floating($('#ren'), `+${heal}`, 'heal'); this.updateHUD(); await sleep(650); await this.enemyOnlyTurn(); }
    async tryEscape() { this.locked = true; this.panel(''); await this.beginPlayerTurn(); const live = this.enemies.filter(e => e.alive), avg = live.reduce((s, e) => s + e.stats.spd, 0) / live.length, chance = clamp(.45 + (this.player.stats.agi - avg) * .025, .35, .9); this.setLog('逃走経路を探している……'); await sleep(600); if (Math.random() < chance) { this.finished = true; this.persistVitals(); this.audio.stopMusic(350); this.audio.sfx('escape'); this.flashTitle('ESCAPED', '戦線を離脱'); await sleep(700); this.showResult('ESCAPED', '怪異との戦闘から離脱し、拠点へ帰還した。', 'RETURN TO HIDEOUT', this.battleSummaryHTML()); } else { this.setLog('逃げられない！'); await sleep(450); await this.enemyOnlyTurn(); } }
    async enemyOnlyTurn() { for (const e of this.enemies.filter(e => e.alive)) { await this.enemyAttack(e); if (this.player.hp <= 0) { await this.defeat(); return; } await sleep(300); } this.endPlayerTurn(); this.turn++; this.locked = false; this.updateHUD(); this.showMainCommands(); }
    ...
    async victory() {
      this.finished = true; this.audio.stopMusic(650); this.audio.sfx('victory'); this.flashTitle('VICTORY', 'ALL SHADOWS ELIMINATED'); $('#ren').classList.add('victory'); await sleep(1100);
    ...
    async defeat() {
      ...
      if (this.battleMode === 'noel') { const stats = this.totalStats(); this.profile.flags.noelFirstEncounterCleared = true; this.profile.flags.preNoelBattleWins = Math.max(this.profile.flags.preNoelBattleWins || 0, D.battleProgression.noelEncounterWins); this.profile.flags.postNoelBattleWins = 0; this.profile.currentVitals = { hp: stats.maxHp, mp: stats.maxMp }; this.player.hp = stats.maxHp; this.player.mp = stats.maxMp; this.saveProfile(); this.flashTitle('DEFEAT', 'NOËL — THE ETERNAL JUDGE'); await sleep(1000); this.showResult('DEFEAT', '圧倒的な裁定の前に敗れた。ノエルは姿を消し、全回復して拠点へ帰還した。', 'THE NEXT KEY', '<div class="workshop-unlock"><b>PHANTOM WORKSHOP</b><strong>工房が解放された！</strong><span>敗北の記録を解析し、装備製作機能が使用可能になりました。</span></div>'); return; }
      this.player.hp = 1; this.persistVitals(); if (this.battleMode === 'zenakado') { this.flashTitle('DEFEAT', 'ZENAKADO WINS'); await sleep(1000); this.showResult('DEFEAT', 'ゼナカドに敗れた。カズに救助され拠点へ帰還した。HPは1で応急処置された。', 'CHALLENGE FAILED', this.battleSummaryHTML() || '<div class="boss-result-note">報酬・ドロップなし</div>'); return; } this.flashTitle('GAME OVER', 'MISSION FAILED'); await sleep(1000); this.showResult('GAME OVER', 'カズに救助され、HP 1で拠点へ運び込まれた。', 'RETURN TO HIDEOUT', this.battleSummaryHTML());
    }
    ...
    renderJobPanel(panel) {
      this.syncSkillUnlocks(); const unlocked = this.jobSystemUnlocked(), currentId = this.profile.currentJob, current = D.jobs[currentId], progress = this.profile.jobs[currentId], need = this.jobExpNeeded(progress.level), learnedIds = new Set(['quickSlash', ...(this.profile.learnedCharacterSkills || []), ...(this.profile.learnedJobSkills || [])]), activeIds = new Set(this.profile.activeSkills || []);
      const jobCards = Object.values(D.jobs).map(job => { const p = this.profile.jobs[job.id], available = unlocked || job.id === 'mage', currentJob = job.id === currentId, next = this.jobExpNeeded(p.level), unlocks = Object.entries(job.skillUnlocks || {}).map(([level, skillId]) => `<span class="${p.level >= Number(level) ? 'learned' : ''}">Lv.${level} ${D.skills[skillId]?.name || skillId}</span>`).join(''); return `<article class="job-card ${currentJob ? 'current' : ''} ${available ? '' : 'locked'}"><header><div><small>${job.nameEn}</small><b>${job.name}</b></div><em>${currentJob ? 'CURRENT' : available ? `JOB Lv.${p.level}` : 'LOCKED'}</em></header><p>${job.description}</p><div class="job-card-progress"><i style="width:${next ? 100 * p.exp / next : 100}%"></i></div><strong>JEXP ${next ? `${p.exp} / ${next}` : 'MASTER'}</strong><small>${this.jobBonusText(job.id)}</small><div class="job-unlocks">${unlocks}</div><button data-job-change="${job.id}" ${!available || currentJob ? 'disabled' : ''}>${currentJob ? '選択中' : available ? 'JOB CHANGE' : 'ゼナカド撃破後'}</button></article>`; }).join('');
      const progressionIds = (D.characterSkillProgression || []).map(entry => entry.skillId), jobSkillIds = Object.values(D.jobs).flatMap(job => Object.values(job.skillUnlocks || {})), allSkillIds = ['quickSlash', ...progressionIds, ...jobSkillIds], skillCards = [...new Set(allSkillIds)].map(id => this.skillDetailHTML(id, learnedIds, activeIds)).join(''), activeSlots = Array.from({ length: 4 }, (_, index) => { const id = this.profile.activeSkills[index], skill = D.skills[id]; return `<div class="active-skill-slot ${skill ? 'filled' : ''}"><small>SLOT ${index + 1}</small><b>${skill?.name || '－'}</b><span>${skill?.nameEn || 'EMPTY'}</span></div>`; }).join('');
      panel.innerHTML = `<small>JOB & SKILL BUILD</small><h2>ジョブ</h2><section class="job-current"><div><small>CURRENT JOB</small><strong>${current.name}</strong><span>${current.nameEn} // JOB Lv.${progress.level}</span></div><div><b>Character Lv.${this.profile.level}</b><b>JEXP ${need ? `${progress.exp} / ${need}` : 'MASTER'}</b><span>${this.jobBonusText(currentId)}</span></div></section>${unlocked ? '' : '<p class="job-lock-notice">JOB SYSTEMは独奏卿ゼナカドの初回撃破後に解禁。現在は初期JOB「魔導士」のみ使用できます。</p>'}<section class="job-section"><header><b>JOB SELECT</b><span>各JOBのLv・JEXPは個別に保存</span></header><div class="job-list">${jobCards}</div></section><section class="job-section"><header><b>ACTIVE SKILLS</b><span>固有・習得済みJOBスキル共通 // 最大4枠</span></header><div class="active-skill-slots">${activeSlots}</div><div class="job-skill-list">${skillCards}</div></section>`;
    }
    renderDungeonSelect(panel) {
      panel.hidden = false;
      const available = (D.dungeons || []).filter(d => this.isDungeonUnlocked(d.id));
      const showNew = this.isDungeonUnlocked('dungeon2') && !this.profile.flags.dungeon2NewSeen;
      if (showNew) { this.profile.flags.dungeon2NewSeen = true; this.saveProfile(); }
      panel.innerHTML = `<button class="panel-home" data-menu="home">拠点へ戻る</button><small>DUNGEON SELECT</small><h2>ダンジョン選択</h2><div class="dungeon-select-list">${available.map(d => {
        const isNew = d.id === 'dungeon2' && showNew;
        const progress = d.id === 'dungeon1' ? (() => { const p = this.progressState(); return p.phase === 'complete' ? 'AREA BOSS CLEARED' : `BATTLE ${Math.min(p.wins, p.goal)} / ${p.goal}`; })() : `BATTLE ${this.profile.flags.dungeon2BattleWins || 0}`;
    ...
    renderWorkshop(panel) {
      ...
      const availableTabs = [...D.workshop.tabs, ...(this.unlockedBossSeries().length ? [{ id: 'bossEquipment', name: 'ボス装備', enName: 'BOSS EQUIPMENT' }] : [])]; if (!availableTabs.some(tab => tab.id === this.workshopTab)) this.workshopTab = 'weapon'; const tabs = availableTabs.map(tab => `<button data-workshop-tab="${tab.id}" class="${this.workshopTab === tab.id ? 'active' : ''}"><b>${tab.name}</b><span>${tab.enName}</span></button>`).join('');
    ...
    bonusText(id) {
      const bonuses = this.equipmentDefinition(id)?.bonuses || {}, rows = Object.entries(bonuses);
      return rows.length ? rows.map(([key, value]) => `${statLabels[key] || key.toUpperCase()} ${value >= 0 ? '+' : ''}${value}`).join(' / ') : '補正なし';
    }
    ...
    renderEquipmentPanel(panel) {
      ...
      const slotHtml = slots.map(slot => { const id = this.profile.equipment[slot.id], item = D.items[id]; return `<div class="equipment-slot ${id ? 'filled' : 'empty'}"><span>${slot.name}<small>${slot.enName}</small></span><b>${item?.name || 'なし'}</b></div>`; }).join('');

  ...
  class BattleGame {
    ...
    */
    ...
    async victory() {
      ...
      if (['d4MidBoss', 'd5MidBoss'].includes(this.battleMode)) { const e = D.enemies[this.battleMode], firstClear = !this.isBossDefeated(this.battleMode); this.markBossDefeated(this.battleMode); if (this.battleMode === 'd4MidBoss') this.noteBossRematchSnapshot('d4MidBoss'); this.saveProfile(); const note = firstClear ? `<div class="boss-recipe-unlock"><small>MID BOSS CLEARED</small><b>ROUTE OPEN</b><strong>${e.name} 撃破</strong><span>封鎖された後半ルートが解放された。</span></div>` : ''; this.showResult('VICTORY', `${e.name}を撃破した！`, 'MID BOSS // COMPLETE', `${rewardBlock}${note}`); this.offerRepeatBossMaterialDrop(e, firstClear); return; }
    ...
    dungeonMidBossEntry(dungeonId) {
      ...
      const e = D.enemies[enemyId], rematch = cleared && ['dungeon3', 'dungeon4'].includes(dungeonId) ? this.rematchProgress(enemyId) : null;

  ...
  class BattleGame {
    ...
    */
    ...
    async defeat() {
      ...
      this.flashTitle('GAME OVER', 'MISSION FAILED'); this.showResult('GAME OVER', '戦闘不能になった。広告を見ると、この戦闘へ復帰できる。', 'REVIVE OR RETURN', `${this.battleSummaryHTML() || ''}<div class="q-after-defeat"><p>怪盗の再起で、現在の戦闘へ復帰できます。</p><button data-q-offer="revive">HP50%で復活する<span>Q’S OFFER // 3秒</span></button></div>`); await this.battleSleep(300); window.arseneQOffer?.show?.('revive', { onGrant: () => this.reviveAfterDefeat() });
    }
    showDefeatWithRevive(copy, kicker = 'REVIVE OR RETURN') { this.showResult('GAME OVER', `${copy} 広告を見ると、この戦闘へ復帰できる。`, kicker, `${this.battleSummaryHTML() || '<div class="boss-result-note">報酬・ドロップなし</div>'}<div class="q-after-defeat"><p>怪盗の再起で、現在の戦闘へ復帰できます。</p><button data-q-offer="revive">HP50%で復活する<span>Q’S OFFER // 3秒</span></button></div>`); window.arseneQOffer?.show?.('revive', { onGrant: () => this.reviveAfterDefeat() }); }

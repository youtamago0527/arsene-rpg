(() => {
  ...
  class BattleGame {
    ...
    passiveValue(passive, key = 'rate', baseOverride = null) {
      ...
      if (!n) return base;
      // 既定は全JOB共通で転生1回につき +2%（絶対値）。
      // 以前の「基本値×40%」は、基本値が小さいパッシブほど伸び幅も小さく
      // 転生しても上がった気がしない、という問題があった。
      // 個別の rebirthSteps / rebirthStep / rebirthTable 指定はそちらが優先される。
      const step = Number(e.rebirthSteps?.[key] ?? e.rebirthStatSteps?.[key] ?? e.rebirthStep ?? (this.gb().passiveRebirthStepFlat ?? 0.02));
    ...
    renderSystemPanel(panel) {
      ...
      if (this.systemTab === 'sound') {
        ...
        body = `<div class="system-actions"><button data-watch-opening>WATCH OPENING<span>オープニングを再生</span></button><button class="danger" data-reset-data>DATA RESET<span>セーブデータを消去</span></button></div>
          ...
          ${adPrivacy}<div class="hideout-feature system-info"><article><b>自動セーブ</b><span>ジョブ・武器学・装備・所持品・GOLD・解放状態をこの端末に保存中。</span></article><article><b>Ver.1.0</b><span>DUNGEON 1–5 AVAILABLE</span></article></div>`;
      }
    ...
    rollVitalGrowth() {
      ...
      // MPもHPと同じ考え方：現在の最大MPが一定量増えるごとに、成長へ必要な敵Spark Lvも上がる。
      // さらにMPは「その戦闘でMPを使う行動を1回でも行った場合」のみ判定対象にする。
      const currentMaxMp = this.profile.baseStats?.maxMp || this.player?.stats?.maxMp || 0;
      const mpTier = Math.floor(Math.max(0, currentMaxMp) / (gb.vitalGrowthMpTierSize ?? gb.vitalGrowthHpTierSize ?? 100));
      const requiredSparkLevelForMp = mpTier * (gb.vitalGrowthSparkPerTier ?? 10);
      const mpEligible = !!this.usedMpThisBattle && strongest >= requiredSparkLevelForMp;
      ...
      if (mpEligible && Math.random() < mpRate) out.mp = amt(gb.mpGrowthAmount);
    ...
    skillEquipmentReady(skill) { if (skill?.weaponIndependent) return true; if (skill?.requiresBareFists && !this.usesBareFists()) return false; const required = skill?.requiresWeaponSubtype; return !required || this.equippedWeapon()?.weaponSubtype === required; }
    ...
    syncSkillUnlocks() { const learnedCharacter = new Set(this.profile.learnedCharacterSkills || []), learnedJob = new Set(this.profile.learnedJobSkills || []); (D.characterSkillProgression || []).forEach(entry => { if (this.profile.level >= entry.level) learnedCharacter.add(entry.skillId); }); Object.entries(this.profile.jobs || {}).forEach(([jobId, progress]) => { const job = D.jobs[jobId]; Object.entries(job?.skillUnlocks || {}).forEach(([level, skillId]) => { if (progress.level >= Number(level)) learnedJob.add(skillId); }); }); // 廃止・武器学へ移動した技は旧セーブのJOB技一覧からも取り除く
      [learnedCharacter, learnedJob].forEach(set => set.forEach(id => { if (!D.skills[id]) set.delete(id); })); learnedJob.forEach(id => { if (D.skills[id]?.source === 'weapon') learnedJob.delete(id); }); this.profile.learnedCharacterSkills = [...learnedCharacter]; this.profile.learnedJobSkills = [...learnedJob]; const learnedWeapon = new Set(this.profile.learnedWeaponSkills || []); Object.values(D.skills).filter(skill => skill?.source === 'weapon' && skill.sparkRate == null && skill.requiredWeaponLevel).forEach(skill => { if ((this.profile.weaponMastery?.[skill.weaponType]?.level || 0) >= skill.requiredWeaponLevel) learnedWeapon.add(skill.id); }); this.profile.learnedWeaponSkills = [...learnedWeapon].filter(id => D.skills[id]?.source === 'weapon'); const allowed = new Set(['quickSlash', ...learnedCharacter, ...learnedJob]); this.profile.activeSkills = (this.profile.activeSkills || []).filter(id => allowed.has(id) && D.skills[id]?.type !== 'PASSIVE').slice(0, 4); Object.entries(this.profile.jobs || {}).forEach(([jobId, progress]) => this.reinforceJobPassives(jobId, progress?.level || 1)); }
    ...
    // 旧growthテーブル方式のJOB(双刃士など)は元々Lv1〜20分しか値を持たない。
    // Lv21〜40（限界突破帯）は個別に値を足さず、対応するLv1〜20の値を2倍にして自動延長する。
    //
    // あわせて転生成長ボーナスもここで掛ける。この倍率は applyJobLevelGrowth() 側に
    // しか入っておらず、テーブル方式のJOBだけ何度転生しても成長量が増えていなかった。
    activeJobBonuses(jobId = this.profile.currentJob) {
      ...
      for (let lv = 1; lv <= level; lv++) {
        const direct = job?.growth?.[lv];
        const entry = direct || (lv > 20 ? job?.growth?.[lv - 20] : null);
    ...
    canUseBareFists(jobId = this.profile.currentJob, equipment = this.profile?.equipment) {
      if (this.jobHasTrait('bareFists', jobId)) return true;
      return jobId === 'phantomThief' && (this.profile?.preferredWeaponType === 'martial' || D.weapons?.[equipment?.rightHand]?.weaponType === 'martial');
    }
    usesBareFists() { return this.canUseBareFists() && this.isBareHanded('rightHand'); }
    ...
    */
    ...
    panel(html, layout = 'sub') {
      ...
      if (layout === 'main') requestAnimationFrame(() => {
        const consoleElement = panel.closest('.battle-console'), game = $('#game');
        if (!consoleElement || !game || game.hidden) return;
        game.style.setProperty('--battle-console-stable-height', `${Math.ceil(consoleElement.getBoundingClientRect().height)}px`);
      });
    }
    ...
    */
    ...
    grantJobExp(amount) {
      ...
      const normalLevels = Math.max(0, Math.min(progress.level, 20) - from);
      ...
      this.syncSkillUnlocks();
      const learned = (this.profile.learnedJobSkills || []).filter(id => !learnedBefore.has(id));
      this.saveProfile();
      return { jobId, jobName: job.name, jobNameEn: job.nameEn, exp: gained, from, to: progress.level, learned, statGain, newPassives, reinforcedPassives };
    }
    ...
    workshopUnlockTutorialHTML() {
      return `<section class="workshop-unlock-tutorial"><header><small>PHANTOM WORKSHOP // QUICK GUIDE</small><b>工房の使い方</b></header><div class="workshop-guide-step"><i>01</i><div><b>装備をクラフト</b><span>ダンジョンでドロップした素材を使って、武器・防具を製作できます。</span></div></div><div class="workshop-guide-step danger"><i>02</i><div><b>同じ装備で強化</b><span>同名装備とGOLDを消費して強化。＋3までは成功率100％、＋4以降は失敗すると装備をロストします。</span></div></div><footer>拠点メニューの《工房》から利用できます</footer></section>`;
    }
    ...
    jobDetailHtml(jobId, unlocked, currentId) {
      const j = D.jobs[jobId], p = this.profile.jobs[jobId] || { level: 1, exp: 0 }, avail = this.isJobUnlocked(jobId), isCur = jobId === currentId, cap = this.effectiveJobLevelCap(), need = p.level >= cap ? null : this.jobExpNeeded(p.level), bar = need ? Math.round(100 * p.exp / need) : 100;
    ...
    renderDungeonSelect(panel) {
      ...
      const midBossNode = midBoss ? (() => { const rm = midBoss.rematch, locked = midBoss.cleared && rm && !rm.ready, status = !midBoss.cleared ? '挑戦可能' : locked ? `再戦まであと ${rm.need - rm.done} 戦` : '再戦可能'; return `<div class="dungeon-tree-node boss ${locked ? 'locked' : 'open'}"><i class="tree-line"></i><button data-boss-challenge="${midBoss.key}" ${locked ? 'disabled' : ''}><span>◉</span><div><small>MID BOSS // ${midBoss.enName}</small><b>${midBoss.name}</b><em>${midBoss.title}</em><strong>${status}</strong></div><figure style="background-image:url('${midBoss.sprite}')"></figure></button></div>`; })() : '';
    ...
    dungeonMidBossEntry(dungeonId) {
      ...
      // 中ボス再戦は全ダンジョン共通。D6以降もmidBossIdを設定するだけで5戦再戦になる。
      const e = D.enemies[enemyId], rematch = cleared ? this.rematchProgress(enemyId) : null;
      return { key: enemyId, name: e.name, enName: e.enName || enemyId.toUpperCase(), title: e.title, sprite: e.sprite, cleared, rematch };
    }
    startBossByKey(key) {
      ...
      if (key === 'versicrell') { this.currentDungeonId = 'dungeon3'; this.startVersicrellBoss(); return; }
    ...
    renderStatusPanel(panel, withTabs = false) {
      ...
      const jid = this.profile.currentJob, jst = this.profile.jobs?.[jid] || {}, jlv = jst.level || 1, jcap = this.effectiveJobLevelCap(), jneed = jlv >= jcap ? null : this.jobExpNeeded(jlv), jexp = jst.exp || 0;
    ...
    unequipSlot(slotId) { if (!slotId || !(slotId in this.profile.equipment)) return; const currentWeaponType = D.weapons?.[this.profile.equipment.rightHand]?.weaponType, bareRightHand = slotId === 'rightHand' && this.canUseBareFists(); if (bareRightHand && currentWeaponType === 'martial') this.profile.preferredWeaponType = 'martial'; this.profile.equipment[slotId] = slotId === 'rightHand' && !bareRightHand ? this.defaultWeaponId() : null; if (bareRightHand) this.profile.equipment.leftHand = null; this.equipSlot = null; this.equipWeaponType = null; this.selectedEquipmentId = null; this.saveProfile(); this.audio.sfx('ui'); this.renderMenuSummary(); this.renderMenuPanel('equipment'); }

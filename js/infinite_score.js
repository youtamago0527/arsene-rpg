...
(() => {
  ...
  P.isMapBack = function () { const map=this.isFloorMap(),back=this.isMapBackNode();if(!map||!back)return;map.currentId=back.id;this.isRun().stairsFound=!!back.stairs;this.isRun().pendingChoices=null;this.isLog('行き止まりから前の分岐へ戻った');this.isRenderExplore($('#menu-panel')); };
  ...
  P.isDirectionOverlay = function(mode){
    ...
    if(!next.length)return `<div class="is-route-overlay deadend"><div class="is-route-title">${back?'この先は行き止まり':'すべての道を調べた'}</div>${back?'<button class="is-route-back" data-is-action="map-back"><b>↓</b><span>戻る</span></button>':'<button class="is-route-back" data-is-action="map-remap">再マッピング</button>'}</div>`;
    const choices=this.isRun().pendingChoices||this.isMakeChoices();
    return `<div class="is-route-overlay ${mode==='examine'?'examining':''}"><div class="is-route-title">${mode==='examine'?'道の先の気配':'進行方向を選択'}</div><div class="is-route-list">${choices.map((choice,index)=>{const node=this.isMapNode(choice.nodeId),d=this.isDirectionLabel(node,index);return `<button data-is-choice="${index}" class="is-route-choice"><b>${d.arrow}</b><span>${d.name}へ</span>${mode==='examine'?`<em>${esc(choice.hint)}</em>`:''}</button>`;}).join('')}</div><button class="is-route-close" data-is-action="route-close">×</button></div>`;
  };
  ...
  P.isRenderExplore = function (panel) {
    ...
    panel.innerHTML=`<main class="is-explore-screen" style="--is-explore-bg:url('../${bg}')">
      ...
      ${this.isAtStairs()?'<button class="is-stair-object" data-is-action="descend"><b>階段</b><span>次の階へ降りる</span></button>':''}
  ...
  // ── 探索 / 戦闘 ───────────────────────────────────────────
  P.isDescend=function(){const r=this.isRun();r.floor=Math.min(this.isEffectiveCfg().maxFloor||9999,r.floor+1);r.encounterCountOnFloor=0;r.stairsFound=false;r.pendingChoices=null;r.floorMap=null;this.isExploreOverlayMode=null;this.isCreateFloorMap();this.isLog(`FLOOR ${r.floor}へ降下`);this.isRenderExplore($('#menu-panel'));};
  ...
  P.isResolveChoice=function(index){const r=this.isRun(),choice=r.pendingChoices?.[index];if(!choice)return;r.pendingChoices=null;this.isExploreOverlayMode=null;const node=choice.nodeId&&this.isMapNode(choice.nodeId);if(node){this.isFloorMap().currentId=node.id;node.visited=true;r.stairsFound=!!node.stairs;}const forced=r.forcedEvent;if(forced){choice.type=forced;r.forcedEvent=null;if(node)node.type=forced;}if(node&&!['encounter','rare'].includes(choice.type))node.cleared=true;const direction=this.isDirectionLabel(node,index).name;this.isLog(`${direction}の道へ進む → ${choice.type}`);this.isResolveEvent(choice.type);};

...
(() => {
  ...
  P.isBagDetailHtml=function(x){
    ...
    return `<section class="is-bag-detail${isEquipped?' equipped':''}" style="--bag-accent:${rar.color||'#52d9ff'}"><header><div><small>${esc(item.slot||item.category||x.kind||'ITEM')}</small><h3>${esc(name)} ${x.plus?`+${x.plus}`:''}</h3></div><strong>${isEquipped?'EQUIPPED':esc(rar.name||'ITEM')}</strong></header><p>${esc(item.description||'奏廊で入手した戦利品。生還すると持ち帰ることができる。')}</p>${combat?`<div class="is-bag-stats">${combat}</div>`:''}${ops?`<div class="is-bag-ops"><small>OPTION EFFECTS</small><ul>${ops}</ul></div>`:''}<div class="is-bag-detail-actions">${item.category==='equipment'?(isEquipped?'<button class="equipped" disabled aria-disabled="true">装備中</button>':`<button class="primary" data-is-equip="${x.uid}">装備する</button>`):''}${item.effect?`<button class="primary" data-is-use="${x.uid}">使用する</button>`:''}<button class="danger" data-is-drop="${x.uid}">捨てる</button></div></section>`;
  };
  P.isRenderBag = function(panel){
    const r=this.isRun();panel.dataset.panel='infinite-score-bag';
    const equipped=new Set(Object.values(r.equipment||{}).filter(Boolean)),rows=r.lootBag.map((x,index)=>{const item=D().items[x.itemId]||{},rar=this.isCfg().rarity[x.rarity]||{},name=x.otherWorldGear&&this.owgName?this.owgName(x):(item.name||x.itemId),isEquipped=equipped.has(x.uid);return `<article class="is-bag-item${isEquipped?' equipped':''}" style="--bag-accent:${rar.color||'#52d9ff'}"><button type="button" class="is-bag-row" data-is-bag-select="${x.uid}" aria-expanded="false"><i>${String(index+1).padStart(2,'0')}</i><span><small>${esc(item.slot||item.category||x.kind||'ITEM')}</small><b>${esc(name)} ${x.plus?`+${x.plus}`:''}</b>${isEquipped?'<mark>装備中</mark>':''}</span><em>${isEquipped?'EQUIPPED':esc(rar.name||'ITEM')}</em><strong>▼</strong></button><div class="is-bag-inline-detail" hidden>${this.isBagDetailHtml(x)}</div></article>`;}).join('');

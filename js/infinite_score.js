...
(() => {
  ...
  P.isForgeTransfer=function(sourceUid,targetUid=null,opIndex=0){
    ...
    const r=this.isRun(),cost=100+chosen.rank*50,same=target.ops.findIndex(o=>o.key===chosen.key),combined=same>=0?this.isForgeCombinedOp(target.ops[same],chosen):null;
    if(r.dungeonGold<cost){alert(`奏貨が足りません。（必要 ${cost}）`);return;}
    const replace=same<0&&target.ops.length>=4?Number(prompt(`上書きするOP\n${target.ops.map((o,i)=>`${i+1}: ${this.isCfg().opLabels[o.key]||o.key} R${o.rank}`).join('\n')}`))-1:-1;
    ...
    if(!confirm(`${this.isForgeName(src)}を消滅させ、OPを${this.isForgeName(target)}へ移植しますか？\n${resultText}必要 ${cost}奏貨`))return;
    r.dungeonGold-=cost;
    if(same>=0)target.ops[same]=combined;else if(replace>=0)target.ops[replace]=copy(chosen);else target.ops.push(copy(chosen));

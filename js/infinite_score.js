...
(() => {
  ...
  P.isSell=function(uid){const r=this.isRun(),x=this.isBagEntry(uid);if(!x)return;Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===uid)delete r.equipment[s]});r.dungeonGold+=this.isSellPrice(x);r.lootBag.splice(r.lootBag.indexOf(x),1);this.isLog(`売却：${D().items[x.itemId]?.name||x.itemId}`);this.isRenderShopSell($('#menu-panel'));};
  ...
  P.isRenderWorkshop=function(panel){const r=this.isRun(),gear=this.isWorkshopGear();panel.dataset.panel='infinite-score-workshop';panel.innerHTML=`<section class="is-ow-workshop"><header><small>RARE ENCOUNTER // OTHER WORLD FORGE</small><h2>異世界工房</h2><p>迷宮の狭間にだけ現れる工房。ここを離れるまで異世界装備を加工できる。</p><strong>${r.dungeonGold.toLocaleString()}<em> 奏貨</em></strong></header><div class="is-ow-workshop-rules"><span>強化</span><span>同型合成</span><span>OP移植</span><span>OP削除</span></div><div class="is-ow-workshop-list">${gear.map(x=>{const item=D().items[x.itemId]||{},name=this.owgName?.(x)||item.name||x.itemId,cost=80+(x.plus||0)*60,ops=(x.ops||[]).map(o=>`${esc(this.isCfg().opLabels[o.key]||o.key)} R${o.rank}`).join(' / ')||'OPなし';return `<article><header><div><small>${'★'.repeat(x.stars||3)} / QUALITY ${x.quality||100}</small><b>${esc(name)} +${x.plus||0}</b></div><em>${esc(item.slot||'EQUIPMENT')}</em></header><div class="is-forge-stats">${this.isForgeStatPreview(x)}</div><p>${ops}</p><div><button data-is-forge="${x.uid}">強化 <span>${cost}奏貨</span></button><button data-is-forge-merge="${x.uid}">合成</button><button data-is-forge-transfer="${x.uid}" ${x.ops?.length?'':'disabled'}>OP移植元</button><button data-is-forge-delete="${x.uid}" ${x.ops?.length?'':'disabled'}>OP削除 <span>50奏貨</span></button></div></article>`;}).join('')||'<p class="is-shop-empty">加工できる異世界装備を持っていない。</p>'}</div><button class="is-shop-exit" data-is-action="workshop-exit">◀ 工房を離れる</button></section>`;};
  P.isForgeEnhance=async function(uid){const r=this.isRun(),x=this.isBagEntry(uid);if(!x?.otherWorldGear)return;const cost=80+(x.plus||0)*60;if(r.dungeonGold<cost){window.arseneStartFlow?.toast?.('奏貨が足りません。');return;}const next=(x.tuneLevel||0)+1,stats=(this.isForgeStatPreview(x,next).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim()),ok=await this.isForgeChoice({title:`${this.owgName?.(x)||D().items[x.itemId]?.name} +${x.plus||0} → +${next}`,copyText:`${stats}\n必要 ${cost}奏貨`,choices:[{label:'強化する',value:true}]});if(!ok)return;r.dungeonGold-=cost;x.plus=next;x.tuneLevel=next;this.isLog(`異世界工房：${this.owgName?.(x)||D().items[x.itemId]?.name}を+${x.plus}へ強化`);this.isSave();this.audio?.sfx?.('confirm');await this.audio?.unlock?.();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeMerge=async function(uid){const base=this.isBagEntry(uid),candidates=this.isWorkshopGear().filter(x=>x.uid!==uid&&x.itemId===base?.itemId);if(!candidates.length){window.arseneStartFlow?.toast?.('同じ種類の異世界装備がありません。');return;}const mat=await this.isForgeChoice({title:'同型合成素材を選択',choices:candidates.map(x=>({label:`${this.owgName?.(x)||D().items[x.itemId]?.name} +${x.plus||0} / QUALITY ${x.quality||100}`,value:x}))});if(!mat)return;const r=this.isRun();base.plus=(base.plus||0)+1;base.tuneLevel=base.plus;base.quality=Math.min(120,Math.max(base.quality||100,mat.quality||100)+1);const pool=(mat.ops||[]).filter(o=>!base.ops.some(v=>v.key===o.key));if(base.ops.length<4&&pool.length)base.ops.push(copy(pool.sort((a,b)=>b.rank-a.rank)[0]));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===mat.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(mat),1);this.isLog(`異世界工房：同型合成 +${base.plus}`);this.isSave();this.audio?.sfx?.('confirm');this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeTransfer=async function(uid){const src=this.isBagEntry(uid);if(!src?.ops?.length)return;const chosen=await this.isForgeChoice({title:'移植するOP',choices:src.ops.map(o=>({label:`${this.isCfg().opLabels[o.key]||o.key} +${o.value} R${o.rank}`,value:o}))});if(!chosen)return;const targets=this.isWorkshopGear().filter(x=>x.uid!==uid);if(!targets.length){window.arseneStartFlow?.toast?.('移植先の異世界装備がありません。');return;}const target=await this.isForgeChoice({title:'移植先',choices:targets.map(x=>({label:`${this.owgName?.(x)||D().items[x.itemId]?.name} +${x.plus||0}`,value:x}))});if(!target)return;const r=this.isRun(),cost=100+chosen.rank*50;if(r.dungeonGold<cost){window.arseneStartFlow?.toast?.(`奏貨が足りません。（必要 ${cost}）`);return;}const same=target.ops.findIndex(o=>o.key===chosen.key);let replace=-1;if(same<0&&target.ops.length>=4){replace=await this.isForgeChoice({title:'上書きするOP',choices:target.ops.map((o,i)=>({label:`${this.isCfg().opLabels[o.key]||o.key} +${o.value} R${o.rank}`,value:i}))});if(replace===null)return;}const ok=await this.isForgeChoice({title:'OP移植の最終確認',copyText:`${this.owgName?.(src)||D().items[src.itemId]?.name}は素材として消滅します。\n必要 ${cost}奏貨`,choices:[{label:'移植する',value:true}]});if(!ok)return;r.dungeonGold-=cost;if(same>=0)target.ops[same]=copy(chosen);else if(replace>=0)target.ops[replace]=copy(chosen);else target.ops.push(copy(chosen));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===src.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(src),1);this.isLog('異世界工房：OP移植');this.isSave();this.audio?.sfx?.('confirm');this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeDelete=async function(uid){const x=this.isBagEntry(uid),r=this.isRun();if(!x?.ops?.length)return;if(r.dungeonGold<50){window.arseneStartFlow?.toast?.('奏貨が足りません。（必要 50）');return;}const idx=await this.isForgeChoice({title:'削除するOP',copyText:'必要 50奏貨',choices:x.ops.map((o,i)=>({label:`${this.isCfg().opLabels[o.key]||o.key} +${o.value} R${o.rank}`,value:i}))});if(idx===null)return;r.dungeonGold-=50;x.ops.splice(idx,1);this.isLog('異世界工房：OP削除');this.isSave();this.audio?.sfx?.('confirm');this.isRenderWorkshop($('#menu-panel'));};

  document.addEventListener('click',e=>{const g=window.arseneGame;if(!g)return;const button=e.target.closest('[data-is-forge],[data-is-forge-merge],[data-is-forge-transfer],[data-is-forge-delete],[data-is-action="workshop-exit"]');if(!button)return;e.preventDefault();e.stopImmediatePropagation();if(button.matches('[data-is-forge]'))void g.isForgeEnhance(button.dataset.isForge);else if(button.matches('[data-is-forge-merge]'))void g.isForgeMerge(button.dataset.isForgeMerge);else if(button.matches('[data-is-forge-transfer]'))void g.isForgeTransfer(button.dataset.isForgeTransfer);else if(button.matches('[data-is-forge-delete]'))void g.isForgeDelete(button.dataset.isForgeDelete);else{g.isRollStairs();g.renderMenuPanel('infinite-score');}},true);

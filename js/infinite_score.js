...
(() => {
  ...
  P.isRenderBag = function(panel){
    ...
    panel.innerHTML=`${this.isHeader()}<section class="is-bag-screen"><header class="is-bag-title"><div><small>LOOT BAG // PHANTOM STORAGE</small><h2>戦利品バッグ</h2></div><div class="is-bag-title-actions"><strong>${this.isBagUsed()}<span>/ ${this.isBagLimit()}</span></strong><button type="button" data-is-bag-sort ${r.lootBag.length<2?'disabled':''}><i>⇅</i>整頓</button></div></header><div class="is-bag-list">${rows||'<p class="is-bag-empty">戦利品はまだ入っていない。</p>'}</div></section>${this.isToolbar()}`;
  };

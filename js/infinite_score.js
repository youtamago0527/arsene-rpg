...
(() => {
  ...
  P.isForgeEnhance=async function(uid){const r=this.isRun(),x=this.isBagEntry(uid);if(!x?.otherWorldGear)return;const cost=80+(x.plus||0)*60,name=this.isForgeName(x);if(r.dungeonGold<cost){await this.isNotify('奏貨が足りません',`必要 ${cost}奏貨 / 所持 ${r.dungeonGold}奏貨`);return;}if(!await this.isConfirm(`${name}を強化しますか？`,`+${x.plus||0} → +${(x.plus||0)+1}\n必要 ${cost}奏貨`,{kicker:'OTHER WORLD FORGE',confirmLabel:'強化する'}))return;r.dungeonGold-=cost;x.plus=(x.plus||0)+1;x.tuneLevel=x.plus;this.isLog(`異世界工房：${name}を+${x.plus}へ強化`);this.isSave();this.audio?.sfx?.('confirm');this.isRenderWorkshop($('#menu-panel'));};
  ...
  P.isForgeTransfer=async function(sourceUid,targetUid=null,opIndex=0){
    ...
    if(r.dungeonGold<cost){await this.isNotify('奏貨が足りません',`必要 ${cost}奏貨 / 所持 ${r.dungeonGold}奏貨`);return;}
    ...
    if(!await this.isConfirm('OPを移植しますか？',`${this.isForgeName(src)}は消滅します。\n${resultText}必要 ${cost}奏貨`,{kicker:'OPTION TRANSPLANT',confirmLabel:'移植する',danger:true}))return;
    r.dungeonGold-=cost;
  ...
  P.isForgeDelete=async function(uid,index=0){const x=this.isBagEntry(uid),r=this.isRun(),op=x?.ops?.[index];if(!op)return;if(r.dungeonGold<50){await this.isNotify('奏貨が足りません',`必要 50奏貨 / 所持 ${r.dungeonGold}奏貨`);return;}if(!await this.isConfirm('このOPを削除しますか？',`${this.isForgeOpText(op)}\n${this.isForgeName(x)}\n必要 50奏貨`,{kicker:'OPTION DELETE',confirmLabel:'削除する',danger:true}))return;r.dungeonGold-=50;x.ops.splice(index,1);this.isForgeUI.opIndex=0;this.isLog('異世界工房：OP削除');this.isSave();this.isRenderWorkshop($('#menu-panel'));};

  P.isForgeExecute=async function(){const s=this.isForgeState(),x=this.isBagEntry(s.selectedUid);if(!x||s.busy)return;s.busy=true;this.isRenderWorkshop($('#menu-panel'));try{if(s.mode==='enhance')await this.isForgeEnhance(x.uid);else if(s.mode==='merge')this.isForgeMerge(x.uid,s.materialUid);else if(s.mode==='transfer')await this.isForgeTransfer(s.sourceUid,x.uid,s.opIndex);else await this.isForgeDelete(x.uid,s.opIndex);}finally{s.busy=false;if(this.isRun())this.isRenderWorkshop($('#menu-panel'));}};

...
(() => {
  ...
  P.isBagDetailHtml=function(x){
    ...
    const item=D().items[x.itemId]||{},rar=this.isCfg().rarity[x.rarity]||{},values=this.isGearValues(x),equippedUid=item.slot&&this.isRun()?.equipment?.[item.slot],equipped=equippedUid&&equippedUid!==x.uid?this.isBagEntry(equippedUid):null,equippedValues=equipped?this.isGearValues(equipped):{},name=x.otherWorldGear&&this.owgName?this.owgName(x):(item.name||x.itemId),combat=Object.entries(values).filter(([,v])=>v).map(([k,v])=>{const delta=equipped?v-(equippedValues[k]||0):v,state=delta>0?'up':delta<0?'down':'same';return `<span class="${state}"><i>${esc(this.isCfg().opLabels[k]||k)}</i><b>${v}</b><em>${delta>0?'▲':delta<0?'▼':'—'} ${delta>0?'+':''}${delta}</em></span>`;}).join(''),ops=(x.ops||[]).map(o=>`<li><span>${esc(this.isCfg().opLabels[o.key]||o.key)}</span><b>${o.key==='critBonus'?`+${Math.round(o.value*100)}%`:`+${o.value}`}</b><em>R${o.rank}</em></li>`).join('');
    return `<section class="is-bag-detail" style="--bag-accent:${rar.color||'#52d9ff'}"><header><div><small>${esc(item.slot||item.category||x.kind||'ITEM')}</small><h3>${esc(name)} ${x.plus?`+${x.plus}`:''}</h3></div><strong>${esc(rar.name||'ITEM')}</strong></header><p>${esc(item.description||'奏廊で入手した戦利品。生還すると持ち帰ることができる。')}</p>${combat?`<div class="is-bag-stats">${combat}</div>`:''}${ops?`<div class="is-bag-ops"><small>OPTION EFFECTS</small><ul>${ops}</ul></div>`:''}<div class="is-bag-detail-actions">${item.category==='equipment'?`<button class="primary" data-is-equip="${x.uid}">装備する</button>`:''}${item.effect?`<button class="primary" data-is-use="${x.uid}">使用する</button>`:''}<button class="danger" data-is-drop="${x.uid}">捨てる</button></div></section>`;
  };
  P.isRenderBag = function(panel){
    const r=this.isRun();panel.dataset.panel='infinite-score-bag';
    const rows=r.lootBag.map((x,index)=>{const item=D().items[x.itemId]||{},rar=this.isCfg().rarity[x.rarity]||{},name=x.otherWorldGear&&this.owgName?this.owgName(x):(item.name||x.itemId);return `<article class="is-bag-item" style="--bag-accent:${rar.color||'#52d9ff'}"><button type="button" class="is-bag-row" data-is-bag-select="${x.uid}" aria-expanded="false"><i>${String(index+1).padStart(2,'0')}</i><span><small>${esc(item.slot||item.category||x.kind||'ITEM')}</small><b>${esc(name)} ${x.plus?`+${x.plus}`:''}</b></span><em>${esc(rar.name||'ITEM')}</em><strong>▼</strong></button><div class="is-bag-inline-detail" hidden>${this.isBagDetailHtml(x)}</div></article>`;}).join('');
    panel.innerHTML=`${this.isHeader()}<section class="is-bag-screen"><header class="is-bag-title"><div><small>LOOT BAG // PHANTOM STORAGE</small><h2>戦利品バッグ</h2></div><strong>${this.isBagUsed()}<span>/ ${this.isBagLimit()}</span></strong></header><div class="is-bag-list">${rows||'<p class="is-bag-empty">戦利品はまだ入っていない。</p>'}</div></section>${this.isToolbar()}`;
  };
  ...
  P.isSell=function(uid){const r=this.isRun(),x=this.isBagEntry(uid);if(!x)return;const slots=Object.keys(r.equipment).filter(s=>r.equipment[s]===uid),name=D().items[x.itemId]?.name||x.itemId;if(slots.length&&!confirm(`「${name}」は装備中です。\n装備を外して売却しますか？`))return;slots.forEach(s=>delete r.equipment[s]);r.dungeonGold+=this.isSellPrice(x);r.lootBag.splice(r.lootBag.indexOf(x),1);this.isLog(`売却：${name}`);this.isRenderShopSell($('#menu-panel'));};
  ...
  P.isForgeOpText=function(o){if(!o)return'OP なし';const label=this.isCfg().opLabels[o.key]||o.key,value=Math.abs(Number(o.value))<1?`${Math.round(Number(o.value)*100)}%`:o.value;return `${label} +${value} / R${o.rank}`;};
  ...
  P.isForgeModePanel=function(x,state){
    ...
    const r=this.isRun(),mode=state.mode,stats=this.isForgeStats(x),current=x.plus||0,cost=80+current*60,equipped=new Set(Object.values(r.equipment||{}).filter(Boolean));
    ...
    if(mode==='enhance'){
      const can=r.dungeonGold>=cost;
      return `<section class="is-forge-preview"><header><small>ENHANCEMENT PREVIEW</small><h3>${esc(this.isForgeName(x))}</h3><b>+${current} <i>≫</i> +${current+1}</b></header><div class="is-forge-stat-table"><small><span>能力</span><b>現在</b><i></i><strong>強化後</strong><em>差分</em></small>${statRows}</div><p class="is-forge-note">既存仕様ではRUN内強化は確定成功です。強化値が+1されます。</p>${this.isForgeCostHtml(cost,can)}<button class="is-forge-execute" data-is-forge-execute ${can&&!state.busy?'':'disabled'}>${state.busy?'強化中…':'強化する'}<span>${cost} 奏貨</span></button></section>`;
    }
    if(mode==='merge'){
    ...
    if(mode==='transfer'){
      const sources=this.isWorkshopGear().filter(v=>v.uid!==x.uid&&v.ops?.length),src=sources.find(v=>v.uid===state.sourceUid)||sources[0];state.sourceUid=src?.uid||null;const op=src?.ops?.[Math.min(state.opIndex,src.ops.length-1)]||src?.ops?.[0];if(src&&state.opIndex>=src.ops.length)state.opIndex=0;const transferCost=op?100+op.rank*50:0,can=!!op&&r.dungeonGold>=transferCost;
      return `<section class="is-forge-preview"><header><small>OPTION TRANSPLANT</small><h3>OP移植</h3><b>TARGET <i>←</i> SOURCE</b></header><div class="is-forge-pair"><article><small>TARGET / 移植先</small><b>${esc(this.isForgeName(x))}</b><span>${esc((x.ops||[]).map(o=>this.isForgeOpText(o)).join(' / ')||'OP なし')}</span></article><i>←</i><article><small>SOURCE / 提供装備</small><b>${src?esc(this.isForgeName(src)):'提供装備なし'}</b><span>${op?esc(this.isForgeOpText(op)):'移植可能OPなし'}</span></article></div><div class="is-forge-candidates">${sources.map(v=>`<button class="${v.uid===src?.uid?'selected':''}" data-is-forge-source="${v.uid}">${equipped.has(v.uid)?'<i>EQUIPPED</i>':''}<b>${esc(this.isForgeName(v))}</b><span>${v.ops.length} OP</span></button>`).join('')}</div><div class="is-forge-op-list">${src?src.ops.map((o,i)=>`<button class="${state.opIndex===i?'selected':''}" data-is-forge-op="${i}">${esc(this.isForgeOpText(o))}</button>`).join(''):'<p>OPを持つ提供装備がありません。</p>'}</div><div class="is-forge-result"><small>移植後OP</small><b>${op?esc(this.isForgeOpText(op)):'—'}</b></div><p class="is-forge-warning">SOURCE装備は移植後に消滅します。</p>${this.isForgeCostHtml(transferCost,can)}<button class="is-forge-execute" data-is-forge-execute ${can&&!state.busy?'':'disabled'}>${state.busy?'移植中…':'OPを移植する'}<span>${transferCost} 奏貨</span></button></section>`;
    }
    const op=x.ops?.[Math.min(state.opIndex,x.ops.length-1)]||x.ops?.[0];if(x.ops?.length&&state.opIndex>=x.ops.length)state.opIndex=0;const can=!!op&&r.dungeonGold>=50;
    return `<section class="is-forge-preview danger"><header><small>OPTION DELETE</small><h3>OP削除</h3><b>${op?esc(this.isForgeOpText(op)):'削除可能OPなし'}</b></header><div class="is-forge-op-list">${ops}</div><div class="is-forge-result"><small>削除後</small><b>選択OPを削除</b></div>${this.isForgeCostHtml(50,can)}<button class="is-forge-execute danger" data-is-forge-execute ${can&&!state.busy?'':'disabled'}>${state.busy?'削除中…':'このOPを削除する'}<span>50 奏貨</span></button></section>`;
  };
  ...
  P.isRenderWorkshop=function(panel){
    ...
    panel.innerHTML=`<main class="is-forge-screen"><header class="is-forge-header"><button data-is-forge-help aria-label="ヘルプ">?</button><div><small>RARE ENCOUNTER // OTHER WORLD FORGE</small><h2>異世界工房</h2></div><button data-is-action="workshop-exit">工房を出る</button></header><section class="is-forge-resource"><div><small>奏貨</small><b>${r.dungeonGold.toLocaleString()}</b><em>帰還時消滅</em></div><p>ショップ・工房で使うラン内専用通貨</p><div><small>現在階層</small><b>${r.floor}</b><em>FLOOR</em></div></section><nav class="is-forge-tabs">${[['enhance','強化'],['merge','同型合成'],['transfer','OP移植'],['delete','OP削除']].map(([id,label])=>`<button class="${state.mode===id?'active':''}${id==='delete'?' danger':''}" data-is-forge-mode="${id}">${label}</button>`).join('')}</nav><section class="is-forge-picker"><header><b>装備選択</b><span class="${full?'full':''}">バッグ ${used} / ${limit}</span></header><div>${gear.map(x=>this.isForgeGearCard(x,x.uid===selected?.uid,equipped.has(x.uid))).join('')||'<p class="is-forge-empty">異世界装備を所持していません。</p>'}</div></section><section class="is-forge-detail-layout">${this.isForgeSelectedHtml(selected,equipped.has(selected?.uid))}<div class="is-forge-preview-column"><small>PROCESS PREVIEW // 右側プレビュー</small>${this.isForgeModePanel(selected,state)}</div></section>${state.help?`<section class="is-forge-help"><div><header><b>異世界工房ガイド</b><button data-is-forge-help>×</button></header><p><strong>強化</strong> 奏貨を使い、RUN中の装備強化値を+1します。</p><p><strong>同型合成</strong> 同じ種類の装備を素材にして、強化値とQUALITYを上げます。</p><p><strong>OP移植</strong> SOURCE装備を消費し、そのOPをTARGET装備へ移します。</p><p><strong>OP削除</strong> 50奏貨で選択したOPを取り除きます。</p><p><strong>奏貨</strong> 同じRUN中は保持され、帰還時に消滅します。</p></div></section>`:''}</main>`;
  };
  P.isForgeEnhance=function(uid){const r=this.isRun(),x=this.isBagEntry(uid);if(!x?.otherWorldGear)return;const cost=80+(x.plus||0)*60;if(r.dungeonGold<cost){alert('奏貨が足りません。');return;}r.dungeonGold-=cost;x.plus=(x.plus||0)+1;x.tuneLevel=x.plus;this.isLog(`異世界工房：${this.owgName?.(x)||D().items[x.itemId]?.name}を+${x.plus}へ強化`);this.isSave();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeMerge=function(uid,materialUid=null){const base=this.isBagEntry(uid),candidates=this.isWorkshopGear().filter(x=>x.uid!==uid&&x.itemId===base?.itemId),mat=materialUid?candidates.find(x=>x.uid===materialUid):null;if(!mat){alert('同じ種類の異世界装備がありません。');return;}const r=this.isRun();base.plus=(base.plus||0)+1;base.tuneLevel=base.plus;base.quality=Math.min(120,Math.max(base.quality||100,mat.quality||100)+1);const pool=(mat.ops||[]).filter(o=>!base.ops.some(v=>v.key===o.key));if(base.ops.length<4&&pool.length)base.ops.push(copy(pool.sort((a,b)=>b.rank-a.rank)[0]));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===mat.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(mat),1);this.isLog(`異世界工房：同型合成 +${base.plus}`);this.isSave();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeTransfer=function(sourceUid,targetUid=null,opIndex=0){const src=this.isBagEntry(sourceUid),chosen=src?.ops?.[opIndex],target=this.isBagEntry(targetUid);if(!src||!chosen||!target||src.uid===target.uid)return;const r=this.isRun(),cost=100+chosen.rank*50;if(r.dungeonGold<cost){alert(`奏貨が足りません。（必要 ${cost}）`);return;}const same=target.ops.findIndex(o=>o.key===chosen.key),replace=same<0&&target.ops.length>=4?Number(prompt(`上書きするOP\n${target.ops.map((o,i)=>`${i+1}: ${this.isCfg().opLabels[o.key]||o.key} R${o.rank}`).join('\n')}`))-1:-1;if(same<0&&target.ops.length>=4&&!target.ops[replace])return;if(!confirm(`${this.isForgeName(src)}を消滅させ、OPを${this.isForgeName(target)}へ移植しますか？\n必要 ${cost}奏貨`))return;r.dungeonGold-=cost;if(same>=0)target.ops[same]=copy(chosen);else if(replace>=0)target.ops[replace]=copy(chosen);else target.ops.push(copy(chosen));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===src.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(src),1);this.isForgeUI.sourceUid=null;this.isLog('異世界工房：OP移植');this.isSave();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeDelete=function(uid,index=0){const x=this.isBagEntry(uid),r=this.isRun(),op=x?.ops?.[index];if(!op)return;if(r.dungeonGold<50){alert('奏貨が足りません。（必要 50）');return;}if(!confirm(`「${this.isForgeOpText(op)}」を削除しますか？\n${this.isForgeName(x)}\n必要 50奏貨`))return;r.dungeonGold-=50;x.ops.splice(index,1);this.isForgeUI.opIndex=0;this.isLog('異世界工房：OP削除');this.isSave();this.isRenderWorkshop($('#menu-panel'));};

  P.isForgeExecute=function(){const s=this.isForgeState(),x=this.isBagEntry(s.selectedUid);if(!x||s.busy)return;s.busy=true;this.isRenderWorkshop($('#menu-panel'));try{if(s.mode==='enhance')this.isForgeEnhance(x.uid);else if(s.mode==='merge')this.isForgeMerge(x.uid,s.materialUid);else if(s.mode==='transfer')this.isForgeTransfer(s.sourceUid,x.uid,s.opIndex);else this.isForgeDelete(x.uid,s.opIndex);}finally{s.busy=false;if(this.isRun())this.isRenderWorkshop($('#menu-panel'));}};

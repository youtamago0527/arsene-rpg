...
(() => {
  ...
  P.isForgeModePanel=function(x,state){
    ...
    if(mode==='enhance'){
      const can=this.isForgeHasMaterials(cost);
      return `<section class="is-forge-preview"><header><small>ENHANCEMENT PREVIEW</small><h3>${esc(this.isForgeName(x))}</h3><b>+${current} <i>≫</i> +${current+1}</b></header><div class="is-forge-stat-table"><small><span>能力</span><b>現在</b><i></i><strong>強化後</strong><em>差分</em></small>${statRows}</div><p class="is-forge-note">RUN内の調律は確定成功です。+6以降は異界の核も必要です。</p>${this.isForgeMaterialCostHtml(cost,can)}<button class="is-forge-execute" data-is-forge-execute ${can&&!state.busy?'':'disabled'}>${state.busy?'調律中…':'調律する'}<span>${this.isForgeCostText(cost)}</span></button></section>`;
    }
    if(mode==='merge'){
  ...
  P.isForgeEnhance=async function(uid){const x=this.isBagEntry(uid);if(!x?.otherWorldGear)return;const cost=this.owgTuneCost?.(x)||{shard:3+(x.plus||0)*2,core:(x.plus||0)>=5?1:0},name=this.isForgeName(x);if(!this.isForgeHasMaterials(cost)){await this.isNotify('異界素材が足りません',`必要 ${this.isForgeCostText(cost)}`);return;}if(!await this.isConfirm(`${name}を調律しますか？`,`+${x.plus||0} → +${(x.plus||0)+1}\n必要 ${this.isForgeCostText(cost)}`,{kicker:'OTHER WORLD FORGE',confirmLabel:'調律する'}))return;this.isForgeSpendMaterials(cost);x.plus=(x.plus||0)+1;x.tuneLevel=x.plus;this.isLog(`異世界工房：${name}を+${x.plus}へ調律`);this.isSave();this.audio?.sfx?.('confirm');this.isRenderWorkshop($('#menu-panel'));};

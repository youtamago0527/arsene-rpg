...
(() => {
  ...
  P.isForgeMerge=function(uid){const base=this.isBagEntry(uid),candidates=this.isWorkshopGear().filter(x=>x.uid!==uid&&x.itemId===base?.itemId);if(!candidates.length){alert('同じ種類の異世界装備がありません。');return;}const pick=Number(prompt(`合成素材を選択\n${candidates.map((x,i)=>`${i+1}: ${this.owgName?.(x)||D().items[x.itemId]?.name} +${x.plus||0}`).join('\n')}`))-1;if(!candidates[pick])return;const mat=candidates[pick],r=this.isRun();base.plus=(base.plus||0)+1;base.tuneLevel=base.plus;base.quality=Math.min(120,Math.max(base.quality||100,mat.quality||100)+1);const pool=(mat.ops||[]).filter(o=>!base.ops.some(v=>v.key===o.key));if(base.ops.length<4&&pool.length)base.ops.push(copy(pool.sort((a,b)=>b.rank-a.rank)[0]));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===mat.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(mat),1);this.isLog(`異世界工房：同型合成 +${base.plus}`);this.isSave();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeTransfer=function(uid){const src=this.isBagEntry(uid);if(!src?.ops?.length)return;const oi=Number(prompt(`移植するOP\n${src.ops.map((o,i)=>`${i+1}: ${this.isCfg().opLabels[o.key]||o.key} R${o.rank}`).join('\n')}`))-1,chosen=src.ops[oi];if(!chosen)return;const targets=this.isWorkshopGear().filter(x=>x.uid!==uid);if(!targets.length){alert('移植先の異世界装備がありません。');return;}const ti=Number(prompt(`移植先\n${targets.map((x,i)=>`${i+1}: ${this.owgName?.(x)||D().items[x.itemId]?.name}`).join('\n')}`))-1,target=targets[ti];if(!target)return;const r=this.isRun(),cost=100+chosen.rank*50;if(r.dungeonGold<cost){alert(`奏貨が足りません。（必要 ${cost}）`);return;}const same=target.ops.findIndex(o=>o.key===chosen.key),replace=same<0&&target.ops.length>=4?Number(prompt(`上書きするOP\n${target.ops.map((o,i)=>`${i+1}: ${this.isCfg().opLabels[o.key]||o.key} R${o.rank}`).join('\n')}`))-1:-1;if(same<0&&target.ops.length>=4&&!target.ops[replace])return;if(!confirm(`${this.owgName?.(src)||D().items[src.itemId]?.name}を素材にしてOPを移植しますか？\n必要 ${cost}奏貨`))return;r.dungeonGold-=cost;if(same>=0)target.ops[same]=copy(chosen);else if(replace>=0)target.ops[replace]=copy(chosen);else target.ops.push(copy(chosen));Object.keys(r.equipment).forEach(s=>{if(r.equipment[s]===src.uid)delete r.equipment[s]});r.lootBag.splice(r.lootBag.indexOf(src),1);this.isLog('異世界工房：OP移植');this.isSave();this.isRenderWorkshop($('#menu-panel'));};
  P.isForgeDelete=function(uid){const x=this.isBagEntry(uid),r=this.isRun();if(!x?.ops?.length)return;if(r.dungeonGold<50){alert('奏貨が足りません。（必要 50）');return;}const idx=Number(prompt(`削除するOP\n${x.ops.map((o,i)=>`${i+1}: ${this.isCfg().opLabels[o.key]||o.key} R${o.rank}`).join('\n')}`))-1;if(!x.ops[idx])return;r.dungeonGold-=50;x.ops.splice(idx,1);this.isLog('異世界工房：OP削除');this.isSave();this.isRenderWorkshop($('#menu-panel'));};

  document.addEventListener('click',e=>{const g=window.arseneGame;if(!g)return;const button=e.target.closest('[data-is-forge],[data-is-forge-merge],[data-is-forge-transfer],[data-is-forge-delete],[data-is-action="workshop-exit"]');if(!button)return;e.preventDefault();e.stopImmediatePropagation();if(button.matches('[data-is-forge]'))g.isForgeEnhance(button.dataset.isForge);else if(button.matches('[data-is-forge-merge]'))g.isForgeMerge(button.dataset.isForgeMerge);else if(button.matches('[data-is-forge-transfer]'))g.isForgeTransfer(button.dataset.isForgeTransfer);else if(button.matches('[data-is-forge-delete]'))g.isForgeDelete(button.dataset.isForgeDelete);else{g.isRollStairs();g.renderMenuPanel('infinite-score');}},true);

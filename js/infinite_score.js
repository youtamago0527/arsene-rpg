...
(() => {
  ...
  P.isRenderExplore = function (panel) {
    ...
    panel.innerHTML=`<main class="is-explore-screen" style="--is-explore-bg:url('../${bg}')">
      ...
      ${this.isExploreMenuHtml()}${notice?'<button class="is-return-unlocked" data-is-action="return-notice-close"><b>RETURN UNLOCKED</b><span>帰還が可能になった</span></button>':''}
    </main>`;
  };
  ...
  P.isRenderShop=function(panel,merchant=false){
    const r=this.isRun(),hasReturn=merchant&&this.isRand()<.12;
    ...
    const stock=[
      product('owPotion20','異界回復薬','HP 20%',25,'＋'),product('owManaPotion20','異界魔力回復薬','MP 20%',30,'✦'),
      product('owPotion40','異界回復薬・中級','HP 40%',65,'＋'),product('owManaPotion40','異界魔力回復薬・中級','MP 40%',75,'✦'),
      product('owPotion60','異界回復薬・高級','HP 60%',125,'＋'),product('owManaPotion60','異界魔力回復薬・高級','MP 60%',140,'✦'),
      product('gear','ランダム装備','MYSTERY GEAR',80,'?'),hasReturn?product('infiniteReturn','RETURN','ESCAPE RELIC',180,'↩'):null
    ].filter(Boolean).join('');
    const sell=r.lootBag.map(x=>`<article class="is-item is-shop-sell"><div><strong>${esc(D().items[x.itemId]?.name||x.itemId)}</strong><small>${x.kind==='equipment'?'EQUIPMENT':'LOOT ITEM'}</small></div><b>${this.isSellPrice(x)}<em> 奏貨</em></b><button data-is-sell="${x.uid}">売却</button></article>`).join('');
    panel.dataset.panel='infinite-score-shop';
    panel.innerHTML=`<section class="is-shop-screen"><header class="is-shop-hero"><small><i></i>${merchant?'WANDERING MERCHANT':'INFINITE SCORE // SHOP'}</small><h2>✦ ${merchant?'彷徨う行商人':'奏廊ショップ'} ✦</h2><p>${merchant?'迷宮を渡る影の商人':'奏廊の狭間でだけ開く商店'}</p><strong>${r.dungeonGold.toLocaleString()}<em> 奏貨</em></strong></header><div class="is-shop-stock"><small>AVAILABLE STOCK</small>${stock}</div><section class="is-shop-trade"><header><div><small>TRADE-IN</small><h3>戦利品を売る</h3></div><span>${r.lootBag.length} ITEMS</span></header><div class="is-list">${sell||'<p class="is-shop-empty">売却できる戦利品はまだない。</p>'}</div></section><button class="is-shop-exit" data-is-action="explore">◀ 奏廊へ戻る</button></section>`;
  };

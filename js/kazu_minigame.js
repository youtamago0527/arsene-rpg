(() => {
  ...
  const S_REWARDS = {
    reijishinshoku: { itemId: 'rebirthArcana', name: '輪廻のアルカナ', stat: '転生', count: 1 },
  ...
  class KazuRhythmGame {
    ...
    finish() {
      ...
      const reward = S_REWARDS[this.track.scoreId || this.track.id];
      const rewardCount = Math.max(1, Number(reward?.count) || 5);
      ...
      if (rank === 'S' && reward && !flags.kazuRhythmSRewards[this.track.id]) {
        ...
        this.game.profile.inventory[reward.itemId] = Math.max(0, Number(this.game.profile.inventory[reward.itemId]) || 0) + rewardCount;
        flags.kazuRhythmSRewards[this.track.id] = true;
        firstSReward = reward;
      }
      this.game.saveProfile();
      this.cover.hidden = false;
      const rewardEffect = firstSReward?.itemId === 'rebirthArcana' ? 'JOB Lv20以降の転生時に1個消費' : firstSReward ? `使用すると《${firstSReward.stat}》が1個につき永久に+1` : '';
      this.setCover(`<div class="kazu-rhythm-card"><small>PERFORMANCE COMPLETE</small><h2 class="kazu-rhythm-rank rank-${rank.toLowerCase()}">${rank}<em>${rank === 'S' ? 'PHANTOM PERFECT' : rank === 'A' ? 'BRILLIANT STEAL' : 'PERFORMANCE RANK'}</em></h2><div class="kazu-rhythm-results"><div><small>SCORE</small><strong>${String(this.score).padStart(7, '0')}</strong></div><div><small>ACCURACY</small><strong>${accuracy.toFixed(1)}%</strong></div><div><small>MAX COMBO</small><strong>${this.maxCombo}</strong></div><div><small>PERFECT</small><strong>${this.counts.perfect}</strong></div></div>${firstSReward ? `<div class="kazu-rhythm-s-reward"><small>FIRST S RANK REWARD</small><strong>${firstSReward.name} ×${rewardCount}</strong><span>${rewardEffect}</span></div>` : rank === 'S' && reward ? `<div class="kazu-rhythm-s-reward claimed"><small>S RANK CLEARED</small><strong>初回報酬は獲得済み</strong></div>` : ''}<button type="button" data-rhythm-action="start">REPLAY</button><button type="button" class="secondary" data-rhythm-action="exit">拠点へ戻る</button></div>`);
    }
